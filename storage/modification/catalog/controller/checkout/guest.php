...
class ControllerCheckoutGuest extends Controller {
	public function index() {
		...
		if ($this->config->get('captcha_' . $this->config->get('config_captcha') . '_status') && in_array('guest', (array)$this->config->get('config_captcha_page'))) {
			...
			$data['captcha'] = '';
              ...
              $data['custom_field_document_id'] = 2;
              $data['custom_number_id'] = 3;    

              pr( $data['custom_field_document_id']  );  die; 
	...
	public function save() {
		...
		if (
           $this->config->get('config_customer_price') || $this->cart->hasDownload()) {
		}
		...
		if (!$json) {
			...
			$this->session->data['guest']['email'] = $this->request->post['email'];
              ...
              $this->session->data['guest']['password'] = $this->request->post['password'];                        
              $this->session->data['guest']['confirm'] = $this->request->post['confirm'];                     

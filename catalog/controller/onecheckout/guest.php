...
class ControllerOneCheckoutGuest extends Controller {
	...
	public function email() {
	    ...
	    $data['exists'] = html_entity_decode($this->url->link('onecheckout/guest/exists'));

	    return $this->response->setOutput($this->load->view('one_checkout/email', $data));
	}
	...
	public function exists() {
		...
		if (empty($this->request->post['email']) || !filter_var($this->request->post['email'], FILTER_VALIDATE_EMAIL)) {
			$json['invalid'] = 'E-mail inválido';
		}

		$query = $this->db->query(" SELECT COUNT(email) as total FROM ".DB_PREFIX."customer WHERE email = '".$this->request->post['email']."' ");

		$json['status'] = ($query->row['total'] > 0);

		$this->session->data['checkout_email'] = $this->request->post['email'];
	...
	public function index() {
		...
		} elseif(!empty($this->session->data['checkout_email'])) {
			$data['email'] = $this->session->data['checkout_email'];
		} else {
			$data['email'] = '';
		}
		...
		$data['custom_field_document_id'] = $this->config->get('module_onecheckout_document_id');
		$data['custom_number_id'] = $this->config->get('module_onecheckout_number_id');

		$this->response->setOutput($this->load->view('one_checkout/guest', $data));
	}
